import { useState, useEffect } from 'react';
import {
  getQueue,
  getDepartments,
  updateVisitStep,
  updateVisit,
  deleteQueueEntry,
  createQueueEntry,
  QueueEntry,
  Department,
} from '../lib/api';
import { useAuth } from '../lib/auth';
import {
  TestTube,
  Clock,
  User,
  Send,
  FileText,
  CheckCircle2,
  ArrowRight,
  ClipboardList,
} from 'lucide-react';

export function LabPage() {
  useAuth();
  const [queue, setQueue] = useState<QueueEntry[]>([]);
  const [departments, setDepartments] = useState<Department[]>([]);
  const [selectedEntry, setSelectedEntry] = useState<QueueEntry | null>(null);
  const [loading, setLoading] = useState(false);
  const [labResults, setLabResults] = useState('');
  const [notes, setNotes] = useState('');
  const [transferTo, setTransferTo] = useState<string>('');
  const [endVisit, setEndVisit] = useState(false);

  useEffect(() => {
    fetchData();
    const interval = setInterval(fetchData, 30000);
    return () => clearInterval(interval);
  }, []);

  const fetchData = async () => {
    try {
      const depts = await getDepartments();
      setDepartments(depts.filter((d) => d.is_active));
      const lab = depts.find((d) => d.code === 'LAB');
      if (lab) {
        const data = await getQueue(lab.id);
        setQueue(data.filter((e) => !e.is_called));
      }
    } catch (err) {
      console.error(err);
    }
  };

  const completeLab = async () => {
    if (!selectedEntry) return;
    setLoading(true);

    const labNotes = `Lab Results: ${labResults}\nNotes: ${notes}`;

    try {
      // Update visit step
      const steps = await fetch(`/api/visit-steps/${selectedEntry.visit_id}`).then(r => r.json());
      const currentStep = steps.find((s: any) => s.department_id === selectedEntry.department_id);
      if (currentStep) {
        await updateVisitStep(currentStep.id, {
          status: 'completed',
          completed_at: new Date().toISOString(),
          notes: labNotes,
        });
      }

      if (endVisit) {
        await updateVisit(selectedEntry.visit_id, {
          status: 'completed',
          completed_at: new Date().toISOString(),
          notes: labNotes,
        });

        await deleteQueueEntry(selectedEntry.id);
      } else if (transferTo) {
        const targetDept = departments.find((d) => d.id === transferTo);
        if (targetDept) {
          await updateVisit(selectedEntry.visit_id, {
            current_department_id: targetDept.id,
            status: 'waiting',
            notes: labNotes,
          });

          await deleteQueueEntry(selectedEntry.id);

          await createQueueEntry({
            visit_id: selectedEntry.visit_id,
            department_id: targetDept.id,
          });
        }
      }

      setSelectedEntry(null);
      setLabResults('');
      setNotes('');
      setTransferTo('');
      setEndVisit(false);
      fetchData();
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
    }
  };

  const consultationDept = departments.find((d) => d.code === 'CONSULTATION');

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">Laboratory</h1>
          <p className="text-slate-400">Process lab tests and record results</p>
        </div>
        <div className="flex items-center gap-3 text-sm">
          <span className="flex items-center gap-2 px-3 py-1.5 bg-slate-800 rounded-lg text-slate-300">
            <Clock className="w-4 h-4" />
            {queue.length} waiting
          </span>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-3">
          <h2 className="text-lg font-semibold text-white">Patient Queue</h2>
          {queue.length === 0 ? (
            <div className="bg-slate-800/50 rounded-xl border border-slate-700 p-8 text-center">
              <TestTube className="w-12 h-12 text-slate-600 mx-auto mb-3" />
              <p className="text-slate-400">No patients in lab queue</p>
            </div>
          ) : (
            <div className="space-y-2">
              {queue.map((entry, index) => (
                <div
                  key={entry.id}
                  className={`p-4 rounded-xl border transition-all cursor-pointer ${
                    selectedEntry?.id === entry.id
                      ? 'bg-cyan-600/20 border-cyan-500'
                      : 'bg-slate-800/50 border-slate-700 hover:border-slate-600'
                  }`}
                  onClick={() => setSelectedEntry(entry)}
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="w-8 h-8 bg-cyan-600 rounded-full flex items-center justify-center text-white font-bold text-sm">
                      {index + 1}
                    </div>
                    <span className="text-xs text-slate-400">
                      {new Date(entry.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                    </span>
                  </div>
                  <p className="font-medium text-white">
                    {entry.visit?.patient?.first_name} {entry.visit?.patient?.last_name}
                  </p>
                  <p className="text-sm text-slate-400">Ticket: {entry.visit?.ticket_number}</p>
                </div>
              ))}
            </div>
          )}
        </div>

        <div className="lg:col-span-2">
          {selectedEntry ? (
            <div className="bg-slate-800/50 rounded-xl border border-slate-700 p-6 space-y-6">
              <div>
                <h2 className="text-xl font-semibold text-white">
                  {selectedEntry.visit?.patient?.first_name} {selectedEntry.visit?.patient?.last_name}
                </h2>
                <p className="text-slate-400">
                  MRN: {selectedEntry.visit?.patient?.medical_record_number}
                </p>
              </div>

              {/* Previous Notes */}
              {selectedEntry.visit?.notes && (
                <div className="p-4 bg-slate-900 rounded-lg">
                  <h3 className="text-sm font-medium text-slate-300 mb-2 flex items-center gap-2">
                    <ClipboardList className="w-4 h-4 text-cyan-400" />
                    Previous Notes
                  </h3>
                  <pre className="text-sm text-white whitespace-pre-wrap">{selectedEntry.visit.notes}</pre>
                </div>
              )}

              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">
                  <FileText className="w-4 h-4 inline mr-2 text-cyan-400" />
                  Lab Results
                </label>
                <textarea
                  value={labResults}
                  onChange={(e) => setLabResults(e.target.value)}
                  rows={6}
                  className="w-full bg-slate-900 border border-slate-600 rounded-lg p-3 text-white focus:outline-none focus:ring-2 focus:ring-cyan-500 font-mono text-sm"
                  placeholder="Enter lab test results..."
                />
              </div>

              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">Additional Notes</label>
                <textarea
                  value={notes}
                  onChange={(e) => setNotes(e.target.value)}
                  rows={2}
                  className="w-full bg-slate-900 border border-slate-600 rounded-lg p-3 text-white focus:outline-none focus:ring-2 focus:ring-cyan-500"
                  placeholder="Any observations..."
                />
              </div>

              <div>
                <label className="block text-sm font-medium text-slate-300 mb-3">Next Step</label>
                <div className="grid grid-cols-2 gap-3">
                  <button
                    onClick={() => {
                      setEndVisit(false);
                      setTransferTo(consultationDept?.id || '');
                    }}
                    className={`p-4 rounded-xl border text-left transition-all ${
                      transferTo === consultationDept?.id
                        ? 'bg-cyan-600/20 border-cyan-500'
                        : 'bg-slate-900 border-slate-700 hover:border-slate-600'
                    }`}
                  >
                    <ArrowRight className="w-6 h-6 text-cyan-400 mb-2" />
                    <p className="font-medium text-white">Return to Consultation</p>
                  </button>
                  <button
                    onClick={() => {
                      setEndVisit(true);
                      setTransferTo('');
                    }}
                    className={`p-4 rounded-xl border text-left transition-all ${
                      endVisit
                        ? 'bg-emerald-600/20 border-emerald-500'
                        : 'bg-slate-900 border-slate-700 hover:border-slate-600'
                    }`}
                  >
                    <CheckCircle2 className="w-6 h-6 text-emerald-400 mb-2" />
                    <p className="font-medium text-white">Complete & End Visit</p>
                  </button>
                </div>
              </div>

              <div className="flex gap-4">
                <button
                  onClick={completeLab}
                  disabled={loading || !labResults.trim()}
                  className="flex-1 py-3 bg-cyan-600 text-white rounded-lg font-medium hover:bg-cyan-700 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  <Send className="w-5 h-5" />
                  {endVisit ? 'Complete' : 'Submit Results'}
                </button>
                <button
                  onClick={() => setSelectedEntry(null)}
                  className="px-6 py-3 bg-slate-700 text-white rounded-lg font-medium hover:bg-slate-600 transition-colors"
                >
                  Cancel
                </button>
              </div>
            </div>
          ) : (
            <div className="bg-slate-800/50 rounded-xl border border-slate-700 p-12 text-center">
              <User className="w-16 h-16 text-slate-600 mx-auto mb-4" />
              <p className="text-slate-400">Select a patient from the queue</p>
            </div>
          )}
        </div>
      </div>
    </div>
  );
}
